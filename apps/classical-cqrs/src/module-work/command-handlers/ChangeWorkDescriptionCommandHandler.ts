...
@CommandHandler(ChangeWorkDescriptionCommand)
export class ChangeWorkDescriptionCommandHandler implements ICommandHandler<ChangeWorkDescriptionCommand> {
  constructor(
    private repository: WorkRepository,
    private publisher: EventPublisher
  ) {}

  async execute(command: ChangeWorkDescriptionCommand): Promise<string> {
    ...
    const events = workAggregate.changeDescription(command)
