...
@CommandHandler(ChangeWorkTitleCommand)
export class ChangeWorkTitleCommandHandler implements ICommandHandler<ChangeWorkTitleCommand> {
  constructor(
    private repository: WorkRepository,
    private publisher: EventPublisher
  ) {}

  async execute(command: ChangeWorkTitleCommand): Promise<string> {
    ...
    const events = workAggregate.changeTitle(command)
