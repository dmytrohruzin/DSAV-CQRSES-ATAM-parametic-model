...
export class CarAggregate extends Aggregate {
  ...
  constructor(snapshot: Snapshot<CarAggregate> = null) {
    if (!snapshot) {
      super()
    } else {
      ...
      if (snapshot.state) {
        this.ownerID = snapshot.state.ownerID
        this.vin = snapshot.state.vin
        this.registrationNumber = snapshot.state.registrationNumber
        this.mileage = snapshot.state.mileage
  ...
  replayCarCreatedV1(event: CarCreatedV1) {
    this.id = event.id
  ...
  replayCarMileageRecordedV1(event: CarMileageRecordedV1) {
    this.mileage = event.mileage
  ...
  replayCarOwnerChangedV1(event: CarOwnerChangedV1) {
    this.ownerID = event.ownerID
  ...
  replayCarDeletedV1(event: CarDeletedV1) {
