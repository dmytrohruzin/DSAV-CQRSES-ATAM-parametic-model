...
@CommandHandler(CompleteWorkCommand)
export class CompleteWorkCommandHandler implements ICommandHandler<CompleteWorkCommand> {
  constructor(
    private repository: WorkRepository,
    private publisher: EventPublisher
  ) {}

  async execute(command: CompleteWorkCommand): Promise<string> {
    ...
    const events = workAggregate.complete()
